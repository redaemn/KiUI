describe('widgets popupMenu:', function() {
  ...
  var elem,
    ...
    KIUI_MENU_HOVERDELAY = 100,
    MenuFake = function() {
      var spy =  jasmine.createSpyObj('kendoMenu', [
        'destroy',
        'bind',
        'close'
      ]);
      ...
      spy.options = {
        hoverDelay: KIUI_MENU_HOVERDELAY
      };
  ...
  describe('PopupMenu', function() {
    ...
    beforeEach(function() {
      elem = $('<div><a><span>Trigger</span></a><ul><li>Menu 1</li></ul></div>').appendTo('body');
    ...
    describe('init()', function() {
      ...
      describe('with "openOnHover" option', function() {
        ...
        it('should close() the menu only once when clicking on an item', function() {
          ...
          jasmine.Clock.tick(KIUI_MENU_HOVERDELAY + 1);
    ...
    describe('open()', function() {
      ...
      it('should not open the menu when it is already open', function() {
        popup.open();
      ...
      it('should not trigger open event if the menu is already open', function() {
        popup.open();
    ...
    describe('close()', function() {
      ...
      it('should close the submenus first and only after a certain delay should trigger close event and toggle the menu', function() {
        popup.open();
        popup.close();
        
        expect(popup.menu.close.calls.length).toBe(1);
        expect(popup.trigger.calls.length).toBe(1);
        expect(popup.trigger.calls[0].args[0]).toBe('open');
        expect(popup._menuEl.slideToggle.calls.length).toBe(1);
        
        jasmine.Clock.tick(KIUI_MENU_HOVERDELAY);
        ...
        expect(popup.trigger.calls[1].args[0]).toBe('close');
        expect(popup._menuEl.slideToggle.calls.length).toBe(2);
      });
      
      it('should not close the menu when it is already closed', function() {
        popup.open();
        popup.close();
        jasmine.Clock.tick(KIUI_MENU_HOVERDELAY + 1);
        popup.close();
      ...
      it('should not trigger close event if the menu is already closed', function() {
        popup.open();
        popup.close();
        jasmine.Clock.tick(KIUI_MENU_HOVERDELAY + 1);
        popup.close();
      ...
      it('should not close the menu if the event is cancelled', function() {
        popup.bind('close', function(e) {
          e.preventDefault();
        });
      ...
      describe('called two times in fast sequence', function() {
        
        beforeEach(function() {
          popup.open();
          popup.close();
          popup.close();
          jasmine.Clock.tick(KIUI_MENU_HOVERDELAY * 3);
        });
        
        it('should toggle the menu only one time', function() {
          expect(popup._menuEl.slideToggle.calls.length).toBe(2);
        });
        
        it('should trigger close event only one time', function() {
          expect(popup.trigger.calls.length).toBe(2);
          expect(popup.trigger.calls[0].args[0]).toBe('open');
          expect(popup.trigger.calls[1].args[0]).toBe('close');
        });
